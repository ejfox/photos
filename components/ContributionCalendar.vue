...
const drawCalendar = () => {
  ...
  const tooltip = d3.select(container.value)
    .append('div')
    .attr('class', 'absolute hidden bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 p-2 rounded text-sm pointer-events-none transform -translate-y-full')
  ...
  const cells = svg.selectAll('rect')
    ...
    .attr('fill', (_d: string, i: number) => colorScale.value(props.contributions[i]))
  ...
  // Add month labels
  const months = Array.from(new Set(props.dates.map(d => {
    const date = new Date(d)
...
watch([width, isDark, () => props.dates, () => props.contributions], drawCalendar)
</script>
