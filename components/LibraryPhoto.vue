...
function cloudinaryThumb(rawHref) {
  ...
  if (cropType.value === "fill") {
    return rawHref.replace(
      "upload/",
      `upload/g_object,c_fill,w_${width.value},h_${width.value},ar_1:1,z_1.5/`,
    );
    ...
    return rawHref.replace(
      "upload/",
      `upload/w_${width.value},c_limit,q_auto,f_auto/`,
    );
  }
}
...
function cloudinarySrcset(rawHref) {
  ...
  if (cropType.value === "fill") {
    return [
      rawHref.replace(
        "upload/",
        `upload/g_object,c_fill,w_${baseWidth},h_${baseWidth},ar_1:1,z_1.5,dpr_1.0/`,
      ) + " 1x",
      rawHref.replace(
        "upload/",
        `upload/g_object,c_fill,w_${baseWidth},h_${baseWidth},ar_1:1,z_1.5,dpr_2.0/`,
      ) + " 2x",
      rawHref.replace(
        "upload/",
        `upload/g_object,c_fill,w_${baseWidth},h_${baseWidth},ar_1:1,z_1.5,dpr_3.0/`,
      ) + " 3x",
    ].join(", ");
  } else if (cropType.value === "pad") {
    return [
      rawHref.replace(
        "upload/",
        `upload/c_pad,w_${baseWidth},h_${baseWidth},b_rgb:${imageBgColor.value},dpr_1.0/`,
      ) + " 1x",
      rawHref.replace(
        "upload/",
        `upload/c_pad,w_${baseWidth},h_${baseWidth},b_rgb:${imageBgColor.value},dpr_2.0/`,
      ) + " 2x",
      rawHref.replace(
        "upload/",
        `upload/c_pad,w_${baseWidth},h_${baseWidth},b_rgb:${imageBgColor.value},dpr_3.0/`,
      ) + " 3x",
    ].join(", ");
  } else {
    return [
      rawHref.replace(
        "upload/",
        `upload/w_${baseWidth},c_limit,q_auto,f_auto,dpr_1.0/`,
      ) + " 1x",
      rawHref.replace(
        "upload/",
        `upload/w_${baseWidth},c_limit,q_auto,f_auto,dpr_2.0/`,
      ) + " 2x",
      rawHref.replace(
        "upload/",
        `upload/w_${baseWidth},c_limit,q_auto,f_auto,dpr_3.0/`,
      ) + " 3x",
    ].join(", ");
  }
}
