      ...
      <div class="max-w-7xl mx-auto px-4 py-12 lg:py-16 space-y-16">
        ...
        <div v-else class="space-y-32">
          ...
          <section v-for="[year, months] in Object.entries(groupedArchives).reverse()" :key="year" class="space-y-8">
            <h2 class="text-sm font-mono tracking-wide text-gray-500 dark:text-gray-400">{{ year }}</h2>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
              <NuxtLink v-for="month in months" :key="`${year}-${month.number}`"
                :to="`/archive/${year}/${month.number}`" class="group space-y-2">
                <!-- Clean image presentation without overlays -->
                <div class="aspect-video bg-gray-100 dark:bg-gray-900 overflow-hidden transition-all duration-500">
                ...
                <div class="flex justify-between items-baseline pt-2">
                  ...
                  <span class="text-xs font-mono text-gray-500 dark:text-gray-400">{{ month.photoCount }}</span>
                </div>
              </NuxtLink>
            </div>
          </section>
