<template>
  <div class="dark:bg-black dark:text-white">
    ...
    <div class="min-h-screen bg-white dark:bg-black text-black dark:text-white">
      <div class="px-4 lg:px-8 py-16">
        <div class="max-w-4xl mx-auto space-y-16">
          <!-- Title -->
          <header class="space-y-4">
            <div class="space-y-2">
              <p
                class="text-xs font-mono uppercase tracking-wider text-gray-500 dark:text-gray-400"
              >
            ...
            <p class="font-mono text-sm text-gray-400 dark:text-gray-500">
          ...
          <div v-else-if="data" class="space-y-16">
            ...
            <section class="grid grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-16">
              ...
              >
                <p
                  class="text-5xl lg:text-6xl font-fjalla tracking-tight tabular-nums"
                >
                  {{ stat.value }}
                </p>
                <p
                ...
                <p
                  v-if="stat.detail"
                  class="text-sm font-mono text-gray-400 dark:text-gray-500"
                >
                  {{ stat.detail }}
                </p>
              </div>
            ...
            <section class="space-y-8">
              ...
              <div class="grid grid-cols-2 gap-x-12">
                <div class="space-y-2">
                  ...
                  <p class="text-sm font-mono text-gray-400 dark:text-gray-500">
                    Current Streak (Days)
                  </p>
                  <p
                    v-if="data.currentStreak.count > 0"
                    class="text-xs font-mono text-gray-400 dark:text-gray-500"
                  >
                    {{ formatDate(new Date(data.currentStreak.startDate)) }} —
                    {{ formatDate(new Date(data.currentStreak.endDate)) }}
                  </p>
                </div>
                <div class="space-y-2">
                  <p
                  ...
                  <p
                    v-if="data.longestStreak.count > 0"
                    class="text-xs font-mono text-gray-400 dark:text-gray-500"
                  >
                    {{ formatDate(new Date(data.longestStreak.startDate)) }} —
                    {{ formatDate(new Date(data.longestStreak.endDate)) }}
                  </p>
                </div>
              </div>
            </section>
            ...
            <section v-if="data.gearStats" class="space-y-16">
              ...
              <div class="space-y-4">
                <div
                  v-for="camera in data.gearStats.cameras"
                  :key="camera.name"
                ...
                >
                  ...
                  <div class="flex items-baseline gap-4">
                    <span
                      class="text-sm font-mono tabular-nums text-gray-500"
                      >{{ camera.count.toString().padStart(4, "0") }}</span
                    >
                    <span
                      class="text-sm font-mono text-gray-400 w-12 text-right"
                      >{{ camera.percentage }}%</span
                    >
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <div
                  v-for="lens in data.gearStats.lenses"
                  :key="lens.name"
                  class="flex items-baseline justify-between border-b border-gray-100 dark:border-gray-800 pb-2"
                >
                  ...
                  <div class="flex items-baseline gap-4">
                    <span
                      class="text-sm font-mono tabular-nums text-gray-500"
                      >{{ lens.count.toString().padStart(4, "0") }}</span
                    >
                    <span
                      class="text-sm font-mono text-gray-400 w-12 text-right"
                      >{{ lens.percentage }}%</span
                    >
                  </div>
                </div>
              </div>
            ...
            <section v-if="data.gearStats?.mostUsedSettings" class="space-y-16">
              ...
              <div class="grid grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div class="space-y-8">
                  ...
                  <div class="space-y-4">
                    <div
                      v-for="setting in data.gearStats.mostUsedSettings
                        .apertures"
                      :key="setting.value"
                    ...
                    >
                      ...
                      <span
                        class="text-sm font-mono tabular-nums text-gray-500"
                        >{{ setting.count.toString().padStart(3, "0") }}</span
                      >
                    </div>
                  </div>
                </div>
