      ...
      while (tween) {
        ...
        if (
          ...
          if (tweenValueType === valueTypes.NUMBER) {
            ...
            const a = clamp(
              tweenModifier(interpolate(fn[3], tn[3], tweenProgress)),
              0,
              1,
            );
            value = `rgba(${r},${g},${b},${a})`;
...
const isValidSVGAttribute = (el, propertyName) => {
  ...
  if (
    propertyName in /** @type {DOMTarget} */ (el).style ||
    propertyName in el
  ) {
...
const convertValueUnit = (el, decomposedValue, unit) => {
  ...
  if (decomposedValue.t === valueTypes.UNIT && valueUnit === unit) {
  ...
  if (!isUnd(cached)) {
    ...
    if (valueUnit in angleUnitsMap) {
      convertedValue =
        (valueNumber * angleUnitsMap[valueUnit]) / angleUnitsMap[unit];
    } else {
      const baseline = 100;
      /** @type {HTMLElement} */
...
class Timer extends Clock {
  ...
  reverse() {
    ...
    this._reversed = +(this._alternate && !(this._iterationCount % 2)
      ? reversed
      : !reversed);
  ...
  then(callback = noop) {
    ...
    return new Promise((r) =>
      this.completed ? r(onResolve()) : (this._resolve = () => r(onResolve())),
    );
  }
...
const generateKeyframes = (keyframes, parameters) => {
  ...
  if (isArr(keyframes)) {
    ...
    const totalDuration = /** @type {Number} */ (
      setValue(parameters.duration, defaults.duration)
    );
  ...
  constructor(
    ...
    const params = /** @type {AnimationParams} */ (
      kParams
        ? mergeObjects(
            generateKeyframes(
              /** @type {DurationKeyframes} */ (kParams),
              parameters,
            ),
            parameters,
          )
    ...
    const tComposition =
      isUnd(composition) && targetsLength >= K
        ...
        : !isUnd(composition)
          ...
          : isNum(animDefaults.composition)
            ? animDefaults.composition
            : compositionTypes[animDefaults.composition];
        ...
        if (isKey(p)) {
          ...
          if (tweenType !== tweenTypes.INVALID) {
            ...
            for (let l = keyframes.length; tweenIndex < l; tweenIndex++) {
              ...
              const tweenComposition = getFunctionValue(
...
function addTlChild(childParams, tl, parsedTLPosition, targets, index, length) {
  ...
  tl.duration = clampInfinity(
    (tl._iterationDuration + tl._loopDelay) * tl._iterationCount -
      tl._loopDelay,
  );
  return tl;
}
...
class Timeline extends Timer {
  ...
  constructor(parameters = {}) {
    ...
    this.defaults =
      parameters && defaultsParams
        ? mergeObjects(defaultsParams, defaults)
        : defaults;
  ...
  add(a1, a2, a3) {
    ...
    if (isAnim || isTimer || isFunc) {
      ...
      if (isAnim) {
        ...
        addTlChild(
          /** @type TimerParams */ (
            isTimer ? a1 : { onComplete: a1, duration: minValue }
          ),
          this,
...
const lerp = (start, end, amount, clock) => {
  ...
  if (activeTimer && activeTimer.deltaTime) {
    delta = 1 - Math.exp(-amount * activeTimer.deltaTime * 0.1);
  }
...
class Draggable {
  ...
  constructor(target, parameters = {}) {
    ...
    this.disabled = [0, 0];
    this.containerBoundsFriction = setValue(parameters.boundsFriction, 0.125);
    if (isUnd(paramX)) {
    ...
    this.$trigger = /** @type {DOMTarget} */ (
      parseTargets(trigger ? trigger : target)[0]
    );
    this.$container = /** @type {DOMTarget} */ (
      container && !this.container ? parseTargets(container)[0] : doc.body
    );
    ...
    if (this.contained) {
      ...
      if (this.container) {
        ...
        this.containerBounds[0] = -round(
          targetRect.left - left + hiddenLeft - cm,
          0,
        );
        this.containerBounds[1] = -round(
          targetRect.top - top + hiddenTop - cm,
          0,
        );
        this.containerBounds[2] = -round(
          targetRect.right - right - hiddenRight + cm,
          0,
        );
        this.containerBounds[3] = -round(
          targetRect.bottom - bottom - hiddenBottom + cm,
          0,
        );
      }
    }
  ...
  handleUp() {
    ...
    if (this.isOutOfBounds(this.containerBounds, cx, cy)) {
      this.setPosition(
        snap(destX, this.snap),
        snap(destY, this.snap),
        duration,
        this.ease,
      );
      ...
      if (this.isOutOfBounds(this.containerBounds, x, y)) {
        ...
        this.setPosition(destX, destY, duration * 0.65, this.ease);
      }
    }
...
const createAnimatable = (targets, parameters) =>
  /** @type {Animatable} */ (new Animatable(targets, parameters));
